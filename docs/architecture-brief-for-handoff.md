...
12. **Separate your data directories by environment from day one.** Sharing a single `data/` directory between local dev and production (via snapshot import/export) is a footgun — a `dev:reset` wipes the only copy of your embeddings. Use `data-development/` locally and `data-production/` on the server, driven by `LIFECOACH_ENV`. Store the env-var separately in `.env.development` (dev) and `.env.production` (server), both gitignored.
