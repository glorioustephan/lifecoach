...
export const ChainOfThoughtContent = ({
  ...
  return (
    <CollapsibleContent
      className={cn(
        "text-fg data-[state=closed]:animate-collapsible-up data-[state=open]:animate-collapsible-down overflow-hidden",
        className
      )}
      {...props}
    >
      <div className="grid grid-cols-[min-content_minmax(0,1fr)] gap-x-4">
        <div className="bg-accent/20 ml-1.75 h-full w-px group-data-[last=true]:hidden" />
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-start group-data-[last=true]:hidden">
        <div className="bg-accent/20 ml-1.75 h-4 w-px" />
      </div>
    </Collapsible>
  )
}
