...
const CandidateCard = ({
  ...
  const rationale =
    "rationale" in candidate.original
      ? candidate.original.rationale
      : undefined;
  const notes =
    "notes" in candidate.original ? candidate.original.notes : undefined;
          ...
          {(rationale || notes) && (
            <Collapsible.Root className="mt-2">
              ...
              <Collapsible.Content className="mt-1.5 space-y-1">
                ...
                {notes && (
                  <p className="text-xs text-fg-faint leading-relaxed italic">
                    {notes}
                  </p>
                )}
              </Collapsible.Content>
...
export const ProposalReviewModal = ({
  ...
  const [newGoalTitle, setNewGoalTitle] = useState(
    parentGoalSuggestion?.title ?? "",
  );
  const [newGoalKind, setNewGoalKind] = useState<
  ...
  return (
    <ModalShell
      open={open}
      onClose={onClose}
      title={`Review ${checkedCount} item${checkedCount !== 1 ? "s" : ""}`}
      description="Review the agent's recommended habits and tasks. Edit titles or cadence inline, optionally group them under a goal, then create the selected items in one step."
            ...
            <div className="space-y-2">
              ...
              >
                <option value="process">
                  Process goal (metric / habit cluster)
                </option>
                <option value="outcome">
                  Outcome goal (bounded, has a due date)
                </option>
                <option value="identity">
                  Identity goal (vote accumulation)
                </option>
              </select>
